"""
Vercel Serverless API — AI Image Analysis
Full DICOM processing, AI orchestration, and clinical report generation.
Migrated from FastAPI backend to Vercel Python serverless functions.
"""
...
def nii_to_numpy(data: bytes) -> np.ndarray:
    try:
        ...
        raise ImportError("nibabel is required for NIfTI processing")
    except Exception as e:
    ...
    def do_POST(self):
        try:
            ...
            if not file_field or not isinstance(file_field, dict):
                self.send_error_response(400, {"error": "No file uploaded"})
                return

            file_bytes = file_field["data"]
            filename = file_field.get("filename", "image.dcm")
            clinical_question = fields.get(
                "clinical_question", "Describe what you see in this medical image."
            )
            tags_str = fields.get("tags", "[]")
            preferred_provider = fields.get("preferred_provider", "modal")
            mode = fields.get("mode", "quick_scan")
            ...
            image_jpeg = numpy_to_jpeg_bytes(image_data)

            body_part = metadata.body_part or "CHEST"
            system_prompt = get_ai_prompt_from_clinical_question(
                clinical_question, body_part
            )
            if mode == "quick_scan":
                system_prompt = f"Answer in 2-3 sentences maximum. {system_prompt}"
            ...
            if not result.get("success"):
                ...
                quick_summary = (
                    generate_quick_scan_summary(raw_output)
                    if mode == "quick_scan"
                    else ""
                )
                estimated_cost = (
                    ...
                    else len(raw_output) / 4 * 0.03 / 1000
                )
                response = {
                    "request_id": request_id,
                    "mode": mode,
